Create one horizontal animation strip for Codex pet `sol-behaviors`, state `running-right`.
...
Identity: same pet in every frame: Preserve the existing sol built-in pet identity exactly; generate only additional natural-life action strips.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
