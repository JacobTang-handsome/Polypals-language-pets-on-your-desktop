Create one horizontal animation strip for Codex pet `ash-behaviors`, state `running-right`.
...
Identity: same pet in every frame: Preserve the existing ash built-in pet identity exactly; generate only additional natural-life action strips.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
