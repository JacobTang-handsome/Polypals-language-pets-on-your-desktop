Create Codex pet row `running-right` for `mousse-behaviors`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing mousse built-in pet identity exactly; generate only additional natural-life action strips.. Preserve silhouette, face, palette, material, proportions, markings, and props.
