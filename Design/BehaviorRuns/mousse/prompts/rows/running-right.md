Create one horizontal animation strip for Codex pet `mousse-behaviors`, state `running-right`.
...
Identity: same pet in every frame: Preserve the existing mousse built-in pet identity exactly; generate only additional natural-life action strips.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
